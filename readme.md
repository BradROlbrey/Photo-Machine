...
The Raspberry Pi and Arduino are on top of the table, while the mechanical part sits on a box below. The ribbon cable for the camera and the wires for the stepper motors come down, and this arrangement keeps them out of the way of the arm as it rotates. The big light strip across the top is to provide even lighting onto the object, and to prevent hard shadows. 

I set the pinecone in the middle, and set the Pi to take 400 pictures of the object - 40 around by 10 up-and-down. Meshroom was able to piece together most of them over the course of 2-3 hours on my 6800k and gtx 970.
...
New upper stepper motor mount. Closeup of the wheel-reel mechanism on lower stepper motor. Lots of slack in the ribbon cable, and the lower stepper's wires, for when the arm spins 360 around the object. Don't mind the masking tape :)
...
Closeup of camera mount (current version) without camera but attached to arm. You can see the eye screws where the fishing line attaches, allowing the lower stepper motor and its wheel/reel to pull the camera up and down the arm.
...
In the future, I would like to improve the frame by incorporating laser-cut plywood, especially for the arm. This would allow the arm to be much bigger instead of being constrained to the area of a 3D printer printbed, allowing the photo machine to take pictures of larger objects.
